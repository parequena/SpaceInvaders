...
bool Map_t::run()
{
	...
	if (m_enemiesKilled == m_totalEnemies) return false;
	
	if(!InputSys_t::update()) return false;

	update(); // Update all components.
	// If we died.
	if (!m_cmpStor->getComponentFromID<HPCmp_t>(m_playerEid)) return false;
...
void Map_t::checkAliveEntites()
{
	...
	for(auto i = 0UL; i < hpSize; ++i)
	{
		...
		--hpSize;
		if (eid == m_lastEnemy) m_lastEnemy = lastAlive;
...
void Map_t::react2Collisions(CollisionCmp_t& lh, CollisionCmp_t& rh) const
{
	...
	if (rh.m_properties & cp::P_isPlayer) std::swap(player, other); // If right is player, swap.
...
void Map_t::checkCollisions() const
{
	...
	for(auto i = 0UL; i < size-1; ++i)
	{
		...
		{
			if ((col1.m_mask & collComps[j].m_mask) == 0) continue;
			
			auto& col2{ collComps[j] };
			
			const auto myID = col1.getCmpID();
			const auto himID = col2.getCmpID();
			if (col1.m_properties & col2.m_properties & CollisionCmp_t::P_isSolid) continue; // If both solid.
			if (myID == himID) continue;
			...
			if (checkInterval(meXLeft, meXRight, himXLeft, himXRight)
				&& checkInterval(meYUp, meYDown, himYUp, himYDown))
			{
